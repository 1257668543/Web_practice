...
const md5 = require('../tools/Jcrypto')
...
const user_regist = async function() {
  ...
  // 检查前端必传的参数是否传了
  const is_avaliable = Jcommon.check_key_words(['user_email', 'user_password', 'user_name'], ctx, 'POST')
  if (is_avaliable == false) return
  ...
  // 验证密码是否正确
  const is_password = Jverify.verify_password(user_password)
